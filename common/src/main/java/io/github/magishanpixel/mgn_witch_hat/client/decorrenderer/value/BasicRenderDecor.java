...
public class BasicRenderDecor implements DecorRenderer.RenderDecor {
    ...
    public BasicRenderDecor(ImmutableList<RenderValue> valList) {
    ...
    public static class Builder {
        ...
        public BasicRenderDecor build() {
            return new BasicRenderDecor(builder.build());
        }
    }
    ...
    public void render(BlockRenderDispatcher blockRenderer, ItemRenderer itemRenderer, MultiBufferSource buffer, PoseStack poseStack, int packedLight, int overlay, Consumer<PoseStack> setPose, ItemStack stack) {
        for (int i = 0; i < rendList.size(); i++) {
            ...
            if (!v.onDebug) {
                ...
                poseStack.mulPose(new Quaternionf().rotationXYZ((float) Math.toRadians(v.rot.x), (float) Math.toRadians(v.rot.y), (float) Math.toRadians(v.rot.z)));
            } else {
                // For hotswapping yuh
                DecorRenderer.setPoseAsDebug(poseStack);
            }

            if (v.modelType != null && v.renderType != null) {
                VertexConsumer vertexConsumer = buffer.getBuffer(v.renderType);
                Model model = HatBakedModels.getModel(v.modelType);

                model.renderToBuffer(poseStack, vertexConsumer, packedLight, overlay);

            } else if (v.blockstate != null) {
                blockRenderer.renderSingleBlock(v.blockstate, poseStack, buffer, packedLight, overlay);
            }
            poseStack.popPose();
        }
    }
